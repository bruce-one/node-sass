...
function getInstalledBinaries() {
  return fs.readdirSync(defaultBinaryPath).map(function(name) { return name.replace(/node-v/, ''); });
}
...
function getBinaryPath() {
  ...
  if (getArgument('--sass-binary-path')) {
    ...
    binaryPath = binary.find(path.resolve(path.join(__dirname, '..', 'package.json')));
  }
